...
export default async function ShopDesignerPage({ params }: Props) {
  ...
  const { data, error } = await supabase
    .from("shops")
    .select("id,slug,name,settings,active")
    .eq("slug", slug)
    .maybeSingle();

  if (error || !data) notFound();
  if (!platformShopAccess(data.settings).customPrint) notFound();
    ...
    return (
      <main
        ...
        style={{
          ...
          "--brand-surface": settings.brand.surfaceColor || "#f4f4ef"
        } as React.CSSProperties}
      >
        <section className="storefront-offline-card">
          {settings.brand.logoUrl
            ? <img src={settings.brand.logoUrl} alt={data.name} />
