...
export async function POST(request: Request) {
  ...
  const suppliedProducts = Array.isArray(body.products) ? (body.products as Record<string, unknown>[]) : [];
  let style = body.style && typeof body.style === "object" ? (body.style as Record<string, unknown>) : {};
  const selectedColorNames: string[] = Array.isArray(body.selectedColors) ? body.selectedColors.map((value: unknown) => String(value)).filter((value: string) => value.length > 0) : [];
  if (!selectedColorNames.length) return NextResponse.json({ error: "Select at least one color." }, { status: 400 });

  const selectedSet = new Set(selectedColorNames);
  let chosen = suppliedProducts.filter((row) => selectedSet.has(String(row.colorName)));

  if (supplier === "sanmar") {
    const styleId = String(style.styleId || "").trim();
    if (!styleId) return NextResponse.json({ error: "A SanMar style number is required." }, { status: 400 });
    ...
    try {
      const liveStyle = await sanmarStyleWithCatalog(supabase, shop.id, connection as any, styleId);
      chosen = liveStyle.variants.filter((variant: any) => selectedSet.has(variant.colorName)).map((variant: any) => {
      ...
      style = {
        ...
        category: liveStyle.cached?.category || style.category || "Apparel"
      };
    } catch (error) {
      return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to refresh the SanMar style before import." }, { status: 502 });
    }
  }

  if (!chosen.length) return NextResponse.json({ error: "The selected colors no longer have active supplier SKUs." }, { status: 400 });
  ...
  const uniqueSelectedColors: string[] = selectedColorNames.filter((name, index, all) => all.indexOf(name) === index && grouped.has(name));
  const colors: ShirtColor[] = uniqueSelectedColors.map((colorName: string) => {
    ...
    return {
      ...
      swatchImageUrl: mediaRow?.swatchImageUrl ? String(mediaRow.swatchImageUrl) : undefined,
      frontImageUrl: mediaRow?.frontImageUrl ? String(mediaRow.frontImageUrl) : undefined,
      backImageUrl: mediaRow?.backImageUrl ? String(mediaRow.backImageUrl) : undefined,
      active: true
    };
  });
  ...
  const sizes = Array.from(new Set(variants.map((variant) => variant.sizeName)));
  const brandName = String(style.brandName || first.brandName || (supplier === "sanmar" ? "SanMar" : "S&S Activewear"));
  const styleName = String(style.styleName || first.styleName || style.styleId || "Blank");
  const name = `${brandName} ${styleName}`.trim();
  const productKindText = `${String(style.category || "")} ${String(style.title || "")} ${styleName}`.toLowerCase();
  ...
  const description = String(style.description || style.title || `${name} imported from ${supplier === "sanmar" ? "SanMar" : "S&S Activewear"}`);
  const baseSlug = slugify(`${name}-${supplier}${targetBusiness === "brand" ? "-brand-source" : ""}`);
  let slug = baseSlug;
  let suffix = 2;
  ...
  const { data, error } = await supabase.from("catalog_products").insert({ organization_id: membership.organization_id, shop_id: shop.id, slug, name, description, active: targetBusiness === "print", configuration }).select("*").single();
  ...
  if (targetBusiness === "brand") {
    const product: CatalogProduct = { ...data, configuration: normalizeConfiguration(data.configuration) };
    const brandSetup = { ...defaultBrandGarmentSetup(product), active: true };
    const { data: brandGarment, error: brandError } = await supabase.from("brand_garments").upsert({ organization_id: membership.organization_id, shop_id: shop.id, source_catalog_product_id: data.id, active: true, configuration: brandSetup }, { onConflict: "shop_id,source_catalog_product_id" }).select("id").single();
    if (brandError) {
      await supabase.from("catalog_products").delete().eq("id", data.id);
      return NextResponse.json({ error: brandError.message }, { status: 400 });
    }
    return NextResponse.json({ product: data, brandGarmentId: brandGarment.id, targetBusiness: "brand", supplier });
  }

  return NextResponse.json({ product: data, targetBusiness: "print", supplier });
}
