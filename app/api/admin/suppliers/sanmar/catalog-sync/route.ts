...
import { syncSanMarCatalog } from "@/lib/sanmar-catalog";
...
export async function POST() {
  ...
  try {
    const result = await syncSanMarCatalog({ supabase, organizationId: membership.organization_id, shopId: shop.id, connection: connection as any });
    return NextResponse.json({ ok: true, ...result });
  } catch (error) {
    return NextResponse.json({ error: error instanceof Error ? error.message : "Unable to sync the SanMar catalog." }, { status: 502 });
  }
}
