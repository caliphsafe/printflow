...
export async function POST(request:Request){try{
 ...
 if(supplier?.provider==='ss-activewear'){
  for(const size of sizes.filter(x=>Number(x.quantity)>0)){const variant=supplier.variants.find(v=>v.colorName===selectedColor.name&&v.sizeName===size.size);if(!variant)return jsonError(`${selectedColor.name} / ${size.size} is unavailable from S&S.`);supplierItems.push({provider:'ss-activewear',sku:variant.sku,skuId:variant.skuId,gtin:variant.gtin,brandName:supplier.brandName,styleName:supplier.styleName,colorName:variant.colorName,sizeName:variant.sizeName,quantity:Number(size.quantity),unitCost:variant.customerPrice,inventorySnapshot:variant.quantity});}
 }
