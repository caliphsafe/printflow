...
export default async function OrderPage({params}:{params:Promise<{id:string}>}){
 ...
 const [{data:o},{data:supplierOrder},{data:connection}]=await Promise.all([
 ...
 {supplierItems.length>0&&<section className="admin-card supplier-order-card"><div className="card-heading"><div><p className="eyebrow">S&S ACTIVEWEAR</p><h2>Blank purchase</h2></div><span className="status-pill">{testMode?'Test mode':'Live ordering'}</span></div><div className="supplier-line-list">{supplierItems.map((x:any)=><div className="supplier-line" key={x.sku}><div><strong>{x.brandName} {x.styleName}</strong><span>{x.colorName} · {x.sizeName}</span></div><div><strong>{x.quantity} pcs</strong><span>{x.sku}</span></div></div>)}</div>{supplierOrder&&<div className="supplier-order-confirmation"><span>S&S order number</span><strong>{(supplierOrder.external_order_numbers||[]).join(', ')||'Submitted'}</strong></div>}<OrderBlanksButton designId={o.id} enabled={o.status==='paid'&&connection?.status==='connected'&&supplierItems.length>0} testMode={testMode} alreadyOrdered={Boolean(supplierOrder)}/></section>}
 </>;
}
