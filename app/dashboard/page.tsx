  ...
  return <>
    ...
    <div className="dashboard-content-grid">
      ...
      <aside className="dashboard-side-stack">
        <section className="admin-card quick-actions-card"><p className="section-kicker">QUICK ACTIONS</p><h2>Keep moving</h2><div className="quick-action-list"><Link href="/dashboard/products"><span>+</span><div><strong>Add a product</strong><small>Colors, sizes and package pricing</small></div></Link><a href={`/s/${shop.slug}`} target="_blank" rel="noreferrer"><span>↗</span><div><strong>Preview designer</strong><small>See the customer experience</small></div></a><Link href="/dashboard/pricing"><span>$</span><div><strong>Global pricing</strong><small>Setup, design services and add-ons</small></div></Link><Link href="/dashboard/integrations"><span>⌁</span><div><strong>Check integrations</strong><small>Payments, commerce and suppliers</small></div></Link></div></section>
        <section className="admin-card pulse-card"><p className="section-kicker">RECENT VALUE</p><strong>${totalValue.toFixed(2)}</strong><p>Verified customer total represented by the latest {recent.length} submissions.</p><div className="pulse-footer"><span>{deliveredResult.count ?? 0} delivered</span><span>{allResult.count ?? 0} total</span></div></section>
      </aside>
    </div>
  </>;
}
