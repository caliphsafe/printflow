...
export default async function DesignsPage(){
  ...
  return <><BrandWorkflowRail active="designs"/><header className="admin-header"><div><p className="eyebrow">BRAND BUILDER · STEP 02</p><h1>Designs, placements & pricing</h1><p>Upload each design once, enable any combination of Front Heart, Front Full, and Back Full, set the price for each version, then approve compatible garments without losing the setup when you switch placements.</p></div></header>{products.length?<BrandDesignManager initialDesigns={designs} categories={(categories||[]) as any} products={products}/>:<section className="admin-card"><h2>Add a Brand garment first</h2><p>Designs need a garment for visual placement approval.</p><a href="/dashboard/brand-garments">Garments & Pricing</a></section>}</>;
}
