...
export default async function AccountPage() {
  const { supabase, user, membership, organization, shop } = await getAdminContext();
  if (!membership || !organization || !shop) return <p>Account setup is incomplete.</p>;
  ...
  return <AccountBillingManager
    plans={(plans || []) as any}
    account={account as any}
    billingConfigured={platformBillingConfigured()}
    profile={{
      fullName: String(user.user_metadata?.full_name || ""),
      businessName: organization.name || shop.name,
      contactEmail: String(shop.settings?.business?.contactEmail || user.email || ""),
      phone: String(shop.settings?.business?.phone || ""),
