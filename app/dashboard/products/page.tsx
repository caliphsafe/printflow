...
export default async function ProductsPage({ searchParams }: Props) {
  ...
  const products: CatalogProduct[] = (data || [])
    .filter((row: any) => row.configuration?.businessScope !== "brand-source")
  ...
  return (
    <>
      <header className="admin-header">
        <div>
          <p className="eyebrow">PRINT SHOP CATALOG</p>
          <h1>Products & production setup</h1>
          <p>These products belong to Custom Print. Brand source garments are intentionally excluded from this workspace.</p>
        </div>
        <div className="admin-header-actions">
          <Link className="secondary-button" href="/dashboard/pricing?returnTo=%2Fdashboard%2Fproducts">Production pricing</Link>
          <Link className="secondary-button" href="/dashboard/suppliers/catalog">Import Print product</Link>
        </div>
      </header>
