...
export default function SupplierCartManager({ initialJobs, providerStates }: { initialJobs: CartJob[]; providerStates: Record<string, ProviderState>; }) {
  ...
  async function remove(job: CartJob) {
    ...
    const response = await fetch(`/api/admin/orders/${job.designId}/blank-draft`, { method: "DELETE", headers: { "Content-Type": "application/json" }, body: JSON.stringify({ provider: job.provider }) });
  ...
  async function place(job: CartJob) {
    ...
    const supplier = providerName(job.provider);
    const mode = job.provider === "ss-activewear" && state?.testMode
      ? "This creates an S&S test order."
      : `This places a real wholesale order in the connected ${supplier} account.`;
    ...
    const response = await fetch(`/api/admin/orders/${job.designId}/order-blanks`, {
      ...
      body: JSON.stringify({ allowBeforePayment: !paid, provider: job.provider })
    });
    const data = await response.json(); setBusyId("");
    ...
    setMessage(`${job.displayId} was submitted to ${supplier}.`);
  }

  if (!jobs.length) return <section className="admin-card supplier-cart-empty"><span>0</span><h2>Your supplier cart is empty.</h2><p>Open a customer order and add its blank requirements. PrintFlow will keep each supplier’s items separated automatically.</p><Link className="primary-button" href="/dashboard/orders">Orders</Link></section>;

  return <div className="supplier-cart-shell">
    <section className="supplier-cart-summary admin-card"><div><p className="section-kicker">PURCHASING</p><h2>{providerName(activeProvider)} cart</h2><p>Review exact SKUs before submitting the wholesale order through the connected supplier account.</p></div><div className="supplier-cart-metrics"><span><small>Jobs</small><strong>{visibleJobs.length}</strong></span><span><small>Pieces</small><strong>{pieces}</strong></span><span><small>Estimated blanks</small><strong>${cost.toFixed(2)}</strong></span></div></section>
    <nav className="supplier-cart-tabs" aria-label="Supplier carts">{providers.map((provider) => <button type="button" key={provider} className={provider === activeProvider ? "active" : ""} onClick={() => setActiveProvider(provider)}><span>{providerName(provider)}</span><small>{jobs.filter((job) => job.provider === provider && !job.ordered).length} active</small></button>)}</nav>
    {message && <div className="success-message supplier-cart-message">{message}</div>}
    <div className="supplier-cart-jobs">{visibleJobs.map((job) => {
      const state = providerStates[job.provider];
      const supported = job.provider === "ss-activewear" || job.provider === "sanmar";
