...
type Style={styleId:string;brandName:string;styleName:string;title:string;description:string;partNumber:string;imageUrl:string;category?:string};
...
export default function SSCatalogImporter({connected}:{connected:boolean}){
 const [open,setOpen]=useState(false),[q,setQ]=useState(''),[styles,setStyles]=useState<Style[]>([]),[style,setStyle]=useState<Style|null>(null),[products,setProducts]=useState<Product[]>([]),[selected,setSelected]=useState<string[]>([]),[loading,setLoading]=useState(false),[message,setMessage]=useState('');
 ...
 return <>
  ...
  {open&&<div className="modal-backdrop" onMouseDown={()=>setOpen(false)}><div className="supplier-modal" onMouseDown={e=>e.stopPropagation()}>
   ...
   {!products.length?<><div className="supplier-search"><input placeholder="Search Gildan 5000, Bella 3001…" value={q} onChange={e=>setQ(e.target.value)} onKeyDown={e=>e.key==='Enter'&&search()}/><button className="primary-button fit-button" disabled={loading} onClick={search}>{loading?'Searching…':'Search'}</button></div>
    <div className="supplier-style-results">{styles.map(item=><button key={item.styleId} className="supplier-style-row" onClick={()=>choose(item)}><div><strong>{item.brandName} {item.styleName}</strong><span>{item.title}</span><small>{item.partNumber}</small></div><span>View colors →</span></button>)}</div></>:
    <><button className="text-button" onClick={()=>{setProducts([]);setSelected([]);setStyle(null)}}>← Back to search</button><div className="import-style-summary"><div><strong>{products[0]?.brandName} {products[0]?.styleName}</strong><span>{style?.title||products[0]?.title}</span></div><small>{colors.length} colors · {new Set(products.map(x=>x.sizeName)).size} sizes · {products.length} SKUs</small></div>
