...
export default function SSActivewearIntegration({ connected: initialConnected, accountHint, initialSettings }: Props) {
 ...
 async function save(){setBusy(true);setMessage("");const r=await fetch('/api/admin/suppliers/ss/connection',{method:'PATCH',headers:{'Content-Type':'application/json'},body:JSON.stringify(settings)});const d=await r.json();setBusy(false);setMessage(r.ok?'Supplier ordering settings saved.':d.error||'Unable to save.');}
 ...
 return <section className="admin-card ss-integration-card">
  ...
  <p>Connect this shop’s own S&amp;S wholesale account to search live products, import colors and sizes, and submit blank orders after customer payment.</p>
  {!connected ? <div className="supplier-connect-form">
   ...
   <div className="integration-actions"><button className="secondary-button" disabled={busy} onClick={disconnect}>Disconnect</button><button className="primary-button fit-button" disabled={busy} onClick={save}>{busy?'Saving…':'Save supplier settings'}</button></div>
  </div>}
