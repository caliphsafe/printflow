...
type Settings = {
  ...
  poEnabled?: boolean;
  shippingAddress?: { customer?: string; attn?: string; address?: string; address2?: string; city?: string; state?: string; zip?: string; residential?: boolean };
};
...
export default function SanMarIntegration({ connected: initialConnected, accountHint, initialSettings }: Props) {
  ...
  const initial: Settings = {
    ...
    sftpUsername: initialSettings?.sftpUsername || initialSettings?.customerNumber || "",
    catalogFilePath: initialSettings?.catalogFilePath || "SanMarPDD/SanMar_EPDD.csv",
    ...
    shippingAddress: { ...emptyAddress, ...(initialSettings?.shippingAddress || {}) }
  };
  ...
  async function syncCatalog(){
    ...
    if(!response.ok)return setMessage(data.error||"Unable to sync the SanMar catalog.");
  ...
  return <section className="admin-card ss-integration-card">
    <div className="card-heading"><div><p className="eyebrow">SANMAR</p><h2>Catalog + Web Services</h2><p>Connected {accountHint||"account"}</p></div><span className="status-pill connected">Connected</span></div>
    <p>The full product browser is fed by SanMar's nightly SFTP catalog. Exact style selection still refreshes real-time Web Services pricing and inventory.</p>
    <h3>Visual catalog access</h3>
    <div className="settings-two-col">
      <label><span>Customer / SFTP username</span><input value={settings.sftpUsername||""} onChange={e=>setSettings({...settings,sftpUsername:e.target.value})}/><small>SanMar documents the FTP username as your customer number.</small></label>
      <label><span>{settings.sftpConfigured?"Replace FTP password":"FTP password"}</span><input type="password" value={sftpPassword} onChange={e=>setSftpPassword(e.target.value)} placeholder={settings.sftpConfigured?"Saved — leave blank to keep":"Separate SanMar FTP password"}/></label>
      <label><span>SFTP host</span><input value={settings.sftpHost||"ftp.sanmar.com"} onChange={e=>setSettings({...settings,sftpHost:e.target.value})}/></label>
      <label><span>SFTP port</span><input type="number" value={settings.sftpPort||2200} onChange={e=>setSettings({...settings,sftpPort:Number(e.target.value)||2200})}/></label>
      <label className="full-field"><span>Catalog file</span><input value={settings.catalogFilePath||"SanMarPDD/SanMar_EPDD.csv"} onChange={e=>setSettings({...settings,catalogFilePath:e.target.value})}/><small>Default is SanMar_EPDD.csv because it includes categories, inventory fields and front/back image URLs.</small></label>
    </div>
    <div className="integration-actions"><button className="primary-button" disabled={syncBusy||busy} onClick={syncCatalog}>{syncBusy?"Syncing SanMar catalog…":"Save + sync SanMar catalog"}</button></div>

    <h3 style={{marginTop:24}}>One-click blank ordering</h3>
    <p className="field-help">SanMar requires separate PO-integration onboarding/testing before production SubmitPO is enabled. Keep this off until SanMar confirms your production account is approved for PO submission.</p>
    <div className="settings-two-col">
      <label className="toggle-row supplier-toggle"><input type="checkbox" checked={settings.poEnabled===true} onChange={e=>setSettings({...settings,poEnabled:e.target.checked})}/><span>SanMar production PO submission is approved</span></label>
      <label><span>Shipping method</span><select value={settings.shippingMethod||"UPS"} onChange={e=>setSettings({...settings,shippingMethod:e.target.value})}><option value="UPS">UPS Ground</option><option value="UPS 2ND DAY">UPS 2nd Day</option><option value="UPS 3RD DAY">UPS 3rd Day</option><option value="UPS NEXT DAY">UPS Next Day</option><option value="USPS PP">USPS Ground Advantage</option><option value="USPS APP">USPS Priority Mail</option><option value="PSST">PSST</option></select></label>
    ...
    <div className="settings-two-col">
      ...
      <label><span>Suite / address 2</span><input value={address.address2||""} onChange={e=>patchAddress("address2",e.target.value)}/></label>
    ...
    {message&&<div className={/unable|failed|required|error/i.test(message)?"error-message catalog-message":"success-message"}>{message}</div>}
    <FloatingSaveBar dirty={dirty} busy={busy} onSave={save} message="Save SanMar catalog and ordering settings." />
  </section>;
}
