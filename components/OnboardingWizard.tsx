...
export default function OnboardingWizard({ existing, selectedPlan = "growth", defaultBusinessName = "" }: { existing: Existing; selectedPlan?: string; defaultBusinessName?: string }) {
  ...
  const [shop, setShop] = useState({ businessName: defaultBusinessName, slug: "", contactEmail: "", phone: "", primaryColor: "#171717", accentColor: "#d8ff5f", headline: "", planCode: selectedPlan });
  ...
  async function bootstrap() {
    ...
    try {
      ...
      setStatus(v=>({...v,shop:"Shop ready"})); setStep(2); router.refresh();
  ...
  return <div className="onboarding-shell">
    ...
    <main className="onboarding-main">
      ...
      {step===1 && <section className="onboarding-card"><div className="onboarding-plan-summary"><span>Plan</span><strong>{selectedPlan.charAt(0).toUpperCase()+selectedPlan.slice(1)}</strong><small>Your trial begins after setup.</small></div><div className="onboarding-grid"><label><span>Business name</span><input required value={shop.businessName} onChange={e=>setShop(v=>({...v,businessName:e.target.value}))}/></label><label><span>Preferred shop URL</span><div className="slug-input"><span>/s/</span><input value={shop.slug} onChange={e=>setShop(v=>({...v,slug:e.target.value.toLowerCase().replace(/[^a-z0-9-]/g,"")}))}/></div></label><label><span>Customer contact email</span><input type="email" value={shop.contactEmail} onChange={e=>setShop(v=>({...v,contactEmail:e.target.value}))}/></label><label><span>Phone</span><input value={shop.phone} onChange={e=>setShop(v=>({...v,phone:e.target.value}))}/></label><label><span>Primary color</span><input type="color" value={shop.primaryColor} onChange={e=>setShop(v=>({...v,primaryColor:e.target.value}))}/></label><label><span>Accent color</span><input type="color" value={shop.accentColor} onChange={e=>setShop(v=>({...v,accentColor:e.target.value}))}/></label><label className="full"><span>Storefront headline</span><input value={shop.headline} onChange={e=>setShop(v=>({...v,headline:e.target.value}))} placeholder="Create something great with us"/></label></div><div className="onboarding-actions"><button className="launch-primary" disabled={busy||!shop.businessName} onClick={bootstrap}>{busy?"Creating shop…":"Create shop and continue"}</button></div></section>}
      {step===2 && <section className="onboarding-stack"><article className="onboarding-card connection-setup-card"><div><span className="live-dot"/><h2>Stripe Checkout</h2><p>Use an `sk_test_` key while validating the workflow, then reconnect with `sk_live_` before launch. Payment updates are connected automatically.</p></div><label><span>Stripe secret key</span><input type="password" value={stripe} onChange={e=>setStripe(e.target.value)} placeholder="sk_live_…"/></label><button className="launch-primary" disabled={busy||!stripe} onClick={()=>connectPayment("stripe")}>{status.payment?.includes("Stripe")?"Stripe connected ✓":"Connect Stripe"}</button></article><article className="onboarding-card connection-setup-card"><div><span className="live-dot"/><h2>Square Checkout</h2><p>Use a Square application personal access token. PrintFlow connects an active location and keeps payment updates in sync automatically.</p></div><label><span>Access token</span><input type="password" value={square.accessToken} onChange={e=>setSquare(v=>({...v,accessToken:e.target.value}))}/></label><label><span>Environment</span><select value={square.environment} onChange={e=>setSquare(v=>({...v,environment:e.target.value}))}><option value="production">Production</option><option value="sandbox">Sandbox</option></select></label><button className="launch-secondary" disabled={busy||!square.accessToken} onClick={()=>connectPayment("square")}>{status.payment?.includes("Square")?"Square connected ✓":"Connect Square"}</button></article><div className="onboarding-actions"><button className="launch-secondary" onClick={()=>setStep(3)}>Continue to supplier</button></div></section>}
      {step===3 && <section className="onboarding-stack"><article className="onboarding-card connection-setup-card"><div><span className="live-dot"/><h2>S&amp;S Activewear</h2><p>Connect the shop’s wholesale account to browse and import the live catalog. Your account pricing, inventory, garment images, and SKUs will be available in the supplier catalog.</p></div><div className="onboarding-grid"><label><span>Account number</span><input value={ss.accountNumber} onChange={e=>setSs(v=>({...v,accountNumber:e.target.value}))}/></label><label><span>API key</span><input type="password" value={ss.apiKey} onChange={e=>setSs(v=>({...v,apiKey:e.target.value}))}/></label></div><button className="launch-primary" disabled={busy||!ss.accountNumber||!ss.apiKey} onClick={connectSupplier}>{status.supplier?"S&S connected ✓":"Connect S&S"}</button></article><div className="onboarding-actions"><button className="launch-secondary" onClick={()=>setStep(4)}>Continue</button></div></section>}
