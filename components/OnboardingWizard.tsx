...
export default function OnboardingWizard({
  ...
  return <div className="onboarding-shell onboarding-shell-v11">
    ...
    <main className="onboarding-main">
      ...
      {step === 6 && <div className="onboarding-stage-layout"><section className="onboarding-card onboarding-primary-card"><div className="onboarding-storefront-layout"><div className="onboarding-grid"><label className="onboarding-color-field"><span>Primary color <InfoTip text="Used for major storefront areas and selected states."/></span><div className="onboarding-color-control"><input className="onboarding-color-swatch" type="color" aria-label="Choose primary color" value={shop.primaryColor} onChange={(event) => setShop((value) => ({ ...value, primaryColor: event.target.value }))}/><input className="onboarding-color-value" value={shop.primaryColor} onChange={(event) => setShop((value) => ({ ...value, primaryColor: event.target.value }))} aria-label="Primary color hex value"/></div></label><label className="onboarding-color-field"><span>Accent color <InfoTip text="Used for buttons, badges, and highlights. Choose a color with strong contrast."/></span><div className="onboarding-color-control"><input className="onboarding-color-swatch" type="color" aria-label="Choose accent color" value={shop.accentColor} onChange={(event) => setShop((value) => ({ ...value, accentColor: event.target.value }))}/><input className="onboarding-color-value" value={shop.accentColor} onChange={(event) => setShop((value) => ({ ...value, accentColor: event.target.value }))} aria-label="Accent color hex value"/></div></label><label className="full"><span>Storefront headline</span><input value={shop.headline} onChange={(event) => setShop((value) => ({ ...value, headline: event.target.value }))} placeholder="Custom apparel, made simple."/></label><label className="full"><span>Introduction</span><textarea rows={4} value={shop.introduction} onChange={(event) => setShop((value) => ({ ...value, introduction: event.target.value }))} placeholder="Choose a garment, upload artwork, and order with confidence."/></label></div><div className="onboarding-mini-preview" style={{ background: shop.primaryColor, color: "#fff" }}><small>YOUR STOREFRONT</small><h2>{shop.headline || "Custom apparel, made simple."}</h2><p>{shop.introduction || "Choose a garment, upload artwork, and order with confidence."}</p><button style={{ background: shop.accentColor }}>Choose a product</button></div></div><div className="storefront-copy-examples"><strong>Helpful headline examples</strong><span>Custom team apparel without the back-and-forth.</span><span>Choose your garment. Upload your design. We handle production.</span></div><div className="onboarding-actions split"><button className="text-button" onClick={() => markStep(7, "storefront")}>Use the default style</button><button className="launch-primary" disabled={busy} onClick={async () => { if (await saveStorefront()) await markStep(7); }}>{busy ? "Saving…" : "Save storefront"}</button></div></section><CoachCard title="Keep the first screen clear"><p>Customers should understand what to do in a few seconds. Lead with the result, then let the product catalog do the work.</p><ul><li>Use one short headline.</li><li>Explain the process in one sentence.</li><li>Choose a high-contrast accent color.</li></ul></CoachCard></div>}
