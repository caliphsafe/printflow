...
const items=[
 {href:"/dashboard",label:"Overview",icon:"home"},{href:"/dashboard/orders",label:"Orders",icon:"orders"},{href:"/dashboard/products",label:"Products",icon:"products"},{href:"/dashboard/suppliers",label:"Suppliers",icon:"suppliers"},{href:"/dashboard/integrations",label:"Integrations",icon:"integrations"},{href:"/dashboard/settings",label:"Shop setup",icon:"settings"}
];
...
export default function DashboardNav(){const pathname=usePathname();return <nav className="admin-nav" aria-label="Dashboard navigation">{items.map(i=>{const active=i.href==='/dashboard'?pathname===i.href:pathname.startsWith(i.href);return <Link key={i.href} href={i.href} className={active?'admin-nav-link active':'admin-nav-link'}><Icon name={i.icon}/><span>{i.label}</span></Link>})}</nav>}
